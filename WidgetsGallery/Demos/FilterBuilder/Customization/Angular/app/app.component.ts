...
export class AppComponent {
    ...
    private static formatValue(value, spaces = 0) {
        if(value && Array.isArray(value[0])) {
            ...
            return "[" + AppComponent.getLineBreak(spaces) + value.map(function(item) {
                return Array.isArray(item[0]) ? AppComponent.formatValue(item, spaces + TAB_SIZE) : JSON.stringify(item);
            }).join("," + AppComponent.getLineBreak(spaces)) + AppComponent.getLineBreak(spaces - TAB_SIZE) + "]";
        }
